...
from mmcv.cnn import ConvModule,DepthwiseSeparableConvModule, Scale, bias_init_with_prob, normal_init
...
class GFL_Light(AnchorHead):
    ...
    def _init_layers(self):
        ...
        self.gfl_cls = nn.ModuleList([nn.Conv2d(self.feat_channels,
                                                ...
                                                1,
                                                padding=0) for _ in self.prior_generator.strides])
        # TODO: if
        ...
        #self.gfl_reg = DepthwiseSeparableConvModule(
    ...
    def forward(self, feats):
        ...
        """
        return multi_apply(self.forward_single, feats,self.cls_convs, self.reg_convs, self.scales,self.gfl_cls,self.gfl_reg)

    def forward_single(self, x,  cls_convs, reg_convs,scale,gfl_cls,gfl_reg):
        ...
        cls_feat = x
        reg_feat = x
        for i in range(len(cls_convs)):
            cls_feat = cls_convs[i](cls_feat)
            reg_feat = reg_convs[i](reg_feat)
